...
const Register: React.FC<RegisterProps> = ({ onBack, onLoginLink, onContinue }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-950 transition-colors duration-300">
      ...
      <div className="px-8 pt-8 pb-10 flex flex-col flex-grow overflow-y-auto no-scrollbar">
        ...
        <Button onClick={onContinue} className="mt-4 mb-8">
          Send verification email
        </Button>

        <div className="text-center mb-8">
            <span className="text-slate-400 dark:text-slate-600 text-sm">Or sign up with</span>
        ...
        <p className="mt-auto text-center text-slate-500 dark:text-slate-500">
          Already have an account? <button onClick={onLoginLink} className="text-[#00D1FF] font-semibold">Log in</button>
        </p>
      </div>
    </div>
  );
